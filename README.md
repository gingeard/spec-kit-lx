...
| **Инварианты как контракт** | «что НЕ меняется» записывается в EARS-форме (`WHEN … system SHALL …`) с привязкой к коду и тесту; при 3+ выносится в `invariants.md` — готовую тест-матрицу |
...
|---|---|---|
| `lx-preset/` | preset (переопределяет ядро) | 8 шаблонов (spec, plan, tasks, constitution, checklist, agent-file + audit-spec, invariants) и обёртки команд `specify`/`implement` |
